/**********************************************************************/
/*   ____  ____                                                       */
/*  /   /\/   /                                                       */
/* /___/  \  /                                                        */
/* \   \   \/                                                       */
/*  \   \        Copyright (c) 2003-2009 Xilinx, Inc.                */
/*  /   /          All Right Reserved.                                 */
/* /---/   /\                                                         */
/* \   \  /  \                                                      */
/*  \___\/\___\                                                    */
/***********************************************************************/

/* This file is designed for use with ISim build 0xfbc00daa */

#define XSI_HIDE_SYMBOL_SPEC true
#include "xsi.h"
#include <memory.h>
#ifdef __GNUC__
#include <stdlib.h>
#else
#include <malloc.h>
#define alloca _alloca
#endif
static const char *ng0 = "/home/ise/COMBINE4MODE/DIV_CLK_4_MODE.v";
static int ng1[] = {1, 0};
static int ng2[] = {0, 0};
static int ng3[] = {2500, 0};
static int ng4[] = {1250, 0};
static int ng5[] = {2, 0};
static int ng6[] = {625, 0};
static int ng7[] = {3, 0};
static int ng8[] = {312, 0};



static void Initial_28_0(char *t0)
{
    char *t1;
    char *t2;

LAB0:    xsi_set_current_line(29, ng0);

LAB2:    xsi_set_current_line(30, ng0);
    t1 = ((char*)((ng1)));
    t2 = (t0 + 1768);
    xsi_vlogvar_assign_value(t2, t1, 0, 0, 25);
    xsi_set_current_line(31, ng0);
    t1 = ((char*)((ng2)));
    t2 = (t0 + 1608);
    xsi_vlogvar_assign_value(t2, t1, 0, 0, 1);

LAB1:    return;
}

static void Always_34_1(char *t0)
{
    char t8[8];
    char *t1;
    char *t2;
    char *t3;
    char *t4;
    char *t5;
    char *t6;
    char *t7;
    char *t9;
    int t10;
    char *t11;
    unsigned int t12;
    unsigned int t13;
    unsigned int t14;
    unsigned int t15;
    unsigned int t16;
    unsigned int t17;
    unsigned int t18;
    unsigned int t19;
    unsigned int t20;
    unsigned int t21;
    unsigned int t22;
    unsigned int t23;
    char *t24;
    char *t25;
    unsigned int t26;
    unsigned int t27;
    unsigned int t28;
    unsigned int t29;
    unsigned int t30;
    char *t31;
    char *t32;

LAB0:    t1 = (t0 + 2928U);
    t2 = *((char **)t1);
    if (t2 == 0)
        goto LAB2;

LAB3:    goto *t2;

LAB2:    xsi_set_current_line(34, ng0);
    t2 = (t0 + 3248);
    *((int *)t2) = 1;
    t3 = (t0 + 2960);
    *((char **)t3) = t2;
    *((char **)t1) = &&LAB4;

LAB1:    return;
LAB4:    xsi_set_current_line(35, ng0);

LAB5:    xsi_set_current_line(36, ng0);
    t4 = (t0 + 1768);
    t5 = (t4 + 56U);
    t6 = *((char **)t5);
    t7 = ((char*)((ng1)));
    memset(t8, 0, 8);
    xsi_vlog_unsigned_add(t8, 32, t6, 25, t7, 32);
    t9 = (t0 + 1768);
    xsi_vlogvar_assign_value(t9, t8, 0, 0, 25);
    xsi_set_current_line(38, ng0);
    t2 = (t0 + 1208U);
    t3 = *((char **)t2);

LAB6:    t2 = ((char*)((ng2)));
    t10 = xsi_vlog_unsigned_case_compare(t3, 2, t2, 32);
    if (t10 == 1)
        goto LAB7;

LAB8:    t2 = ((char*)((ng1)));
    t10 = xsi_vlog_unsigned_case_compare(t3, 2, t2, 32);
    if (t10 == 1)
        goto LAB9;

LAB10:    t2 = ((char*)((ng5)));
    t10 = xsi_vlog_unsigned_case_compare(t3, 2, t2, 32);
    if (t10 == 1)
        goto LAB11;

LAB12:    t2 = ((char*)((ng7)));
    t10 = xsi_vlog_unsigned_case_compare(t3, 2, t2, 32);
    if (t10 == 1)
        goto LAB13;

LAB14:
LAB15:    goto LAB2;

LAB7:    xsi_set_current_line(39, ng0);

LAB16:    xsi_set_current_line(39, ng0);
    t4 = (t0 + 1768);
    t5 = (t4 + 56U);
    t6 = *((char **)t5);
    t7 = ((char*)((ng3)));
    memset(t8, 0, 8);
    t9 = (t6 + 4);
    t11 = (t7 + 4);
    t12 = *((unsigned int *)t6);
    t13 = *((unsigned int *)t7);
    t14 = (t12 ^ t13);
    t15 = *((unsigned int *)t9);
    t16 = *((unsigned int *)t11);
    t17 = (t15 ^ t16);
    t18 = (t14 | t17);
    t19 = *((unsigned int *)t9);
    t20 = *((unsigned int *)t11);
    t21 = (t19 | t20);
    t22 = (~(t21));
    t23 = (t18 & t22);
    if (t23 != 0)
        goto LAB20;

LAB17:    if (t21 != 0)
        goto LAB19;

LAB18:    *((unsigned int *)t8) = 1;

LAB20:    t25 = (t8 + 4);
    t26 = *((unsigned int *)t25);
    t27 = (~(t26));
    t28 = *((unsigned int *)t8);
    t29 = (t28 & t27);
    t30 = (t29 != 0);
    if (t30 > 0)
        goto LAB21;

LAB22:
LAB23:    goto LAB15;

LAB9:    xsi_set_current_line(40, ng0);

LAB31:    xsi_set_current_line(40, ng0);
    t4 = (t0 + 1768);
    t5 = (t4 + 56U);
    t6 = *((char **)t5);
    t7 = ((char*)((ng4)));
    memset(t8, 0, 8);
    t9 = (t6 + 4);
    t11 = (t7 + 4);
    t12 = *((unsigned int *)t6);
    t13 = *((unsigned int *)t7);
    t14 = (t12 ^ t13);
    t15 = *((unsigned int *)t9);
    t16 = *((unsigned int *)t11);
    t17 = (t15 ^ t16);
    t18 = (t14 | t17);
    t19 = *((unsigned int *)t9);
    t20 = *((unsigned int *)t11);
    t21 = (t19 | t20);
    t22 = (~(t21));
    t23 = (t18 & t22);
    if (t23 != 0)
        goto LAB35;

LAB32:    if (t21 != 0)
        goto LAB34;

LAB33:    *((unsigned int *)t8) = 1;

LAB35:    t25 = (t8 + 4);
    t26 = *((unsigned int *)t25);
    t27 = (~(t26));
    t28 = *((unsigned int *)t8);
    t29 = (t28 & t27);
    t30 = (t29 != 0);
    if (t30 > 0)
        goto LAB36;

LAB37:
LAB38:    goto LAB15;

LAB11:    xsi_set_current_line(41, ng0);

LAB46:    xsi_set_current_line(41, ng0);
    t4 = (t0 + 1768);
    t5 = (t4 + 56U);
    t6 = *((char **)t5);
    t7 = ((char*)((ng6)));
    memset(t8, 0, 8);
    t9 = (t6 + 4);
    t11 = (t7 + 4);
    t12 = *((unsigned int *)t6);
    t13 = *((unsigned int *)t7);
    t14 = (t12 ^ t13);
    t15 = *((unsigned int *)t9);
    t16 = *((unsigned int *)t11);
    t17 = (t15 ^ t16);
    t18 = (t14 | t17);
    t19 = *((unsigned int *)t9);
    t20 = *((unsigned int *)t11);
    t21 = (t19 | t20);
    t22 = (~(t21));
    t23 = (t18 & t22);
    if (t23 != 0)
        goto LAB50;

LAB47:    if (t21 != 0)
        goto LAB49;

LAB48:    *((unsigned int *)t8) = 1;

LAB50:    t25 = (t8 + 4);
    t26 = *((unsigned int *)t25);
    t27 = (~(t26));
    t28 = *((unsigned int *)t8);
    t29 = (t28 & t27);
    t30 = (t29 != 0);
    if (t30 > 0)
        goto LAB51;

LAB52:
LAB53:    goto LAB15;

LAB13:    xsi_set_current_line(42, ng0);

LAB61:    xsi_set_current_line(42, ng0);
    t4 = (t0 + 1768);
    t5 = (t4 + 56U);
    t6 = *((char **)t5);
    t7 = ((char*)((ng8)));
    memset(t8, 0, 8);
    t9 = (t6 + 4);
    t11 = (t7 + 4);
    t12 = *((unsigned int *)t6);
    t13 = *((unsigned int *)t7);
    t14 = (t12 ^ t13);
    t15 = *((unsigned int *)t9);
    t16 = *((unsigned int *)t11);
    t17 = (t15 ^ t16);
    t18 = (t14 | t17);
    t19 = *((unsigned int *)t9);
    t20 = *((unsigned int *)t11);
    t21 = (t19 | t20);
    t22 = (~(t21));
    t23 = (t18 & t22);
    if (t23 != 0)
        goto LAB65;

LAB62:    if (t21 != 0)
        goto LAB64;

LAB63:    *((unsigned int *)t8) = 1;

LAB65:    t25 = (t8 + 4);
    t26 = *((unsigned int *)t25);
    t27 = (~(t26));
    t28 = *((unsigned int *)t8);
    t29 = (t28 & t27);
    t30 = (t29 != 0);
    if (t30 > 0)
        goto LAB66;

LAB67:
LAB68:    goto LAB15;

LAB19:    t24 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t24) = 1;
    goto LAB20;

LAB21:    xsi_set_current_line(39, ng0);

LAB24:    xsi_set_current_line(39, ng0);
    t31 = ((char*)((ng1)));
    t32 = (t0 + 1768);
    xsi_vlogvar_assign_value(t32, t31, 0, 0, 25);
    xsi_set_current_line(39, ng0);
    t2 = (t0 + 1608);
    t4 = (t2 + 56U);
    t5 = *((char **)t4);
    memset(t8, 0, 8);
    t6 = (t5 + 4);
    t12 = *((unsigned int *)t6);
    t13 = (~(t12));
    t14 = *((unsigned int *)t5);
    t15 = (t14 & t13);
    t16 = (t15 & 1U);
    if (t16 != 0)
        goto LAB28;

LAB26:    if (*((unsigned int *)t6) == 0)
        goto LAB25;

LAB27:    t7 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t7) = 1;

LAB28:    t9 = (t8 + 4);
    t11 = (t5 + 4);
    t17 = *((unsigned int *)t5);
    t18 = (~(t17));
    *((unsigned int *)t8) = t18;
    *((unsigned int *)t9) = 0;
    if (*((unsigned int *)t11) != 0)
        goto LAB30;

LAB29:    t23 = *((unsigned int *)t8);
    *((unsigned int *)t8) = (t23 & 1U);
    t26 = *((unsigned int *)t9);
    *((unsigned int *)t9) = (t26 & 1U);
    t24 = (t0 + 1608);
    xsi_vlogvar_assign_value(t24, t8, 0, 0, 1);
    goto LAB23;

LAB25:    *((unsigned int *)t8) = 1;
    goto LAB28;

LAB30:    t19 = *((unsigned int *)t8);
    t20 = *((unsigned int *)t11);
    *((unsigned int *)t8) = (t19 | t20);
    t21 = *((unsigned int *)t9);
    t22 = *((unsigned int *)t11);
    *((unsigned int *)t9) = (t21 | t22);
    goto LAB29;

LAB34:    t24 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t24) = 1;
    goto LAB35;

LAB36:    xsi_set_current_line(40, ng0);

LAB39:    xsi_set_current_line(40, ng0);
    t31 = ((char*)((ng1)));
    t32 = (t0 + 1768);
    xsi_vlogvar_assign_value(t32, t31, 0, 0, 25);
    xsi_set_current_line(40, ng0);
    t2 = (t0 + 1608);
    t4 = (t2 + 56U);
    t5 = *((char **)t4);
    memset(t8, 0, 8);
    t6 = (t5 + 4);
    t12 = *((unsigned int *)t6);
    t13 = (~(t12));
    t14 = *((unsigned int *)t5);
    t15 = (t14 & t13);
    t16 = (t15 & 1U);
    if (t16 != 0)
        goto LAB43;

LAB41:    if (*((unsigned int *)t6) == 0)
        goto LAB40;

LAB42:    t7 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t7) = 1;

LAB43:    t9 = (t8 + 4);
    t11 = (t5 + 4);
    t17 = *((unsigned int *)t5);
    t18 = (~(t17));
    *((unsigned int *)t8) = t18;
    *((unsigned int *)t9) = 0;
    if (*((unsigned int *)t11) != 0)
        goto LAB45;

LAB44:    t23 = *((unsigned int *)t8);
    *((unsigned int *)t8) = (t23 & 1U);
    t26 = *((unsigned int *)t9);
    *((unsigned int *)t9) = (t26 & 1U);
    t24 = (t0 + 1608);
    xsi_vlogvar_assign_value(t24, t8, 0, 0, 1);
    goto LAB38;

LAB40:    *((unsigned int *)t8) = 1;
    goto LAB43;

LAB45:    t19 = *((unsigned int *)t8);
    t20 = *((unsigned int *)t11);
    *((unsigned int *)t8) = (t19 | t20);
    t21 = *((unsigned int *)t9);
    t22 = *((unsigned int *)t11);
    *((unsigned int *)t9) = (t21 | t22);
    goto LAB44;

LAB49:    t24 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t24) = 1;
    goto LAB50;

LAB51:    xsi_set_current_line(41, ng0);

LAB54:    xsi_set_current_line(41, ng0);
    t31 = ((char*)((ng1)));
    t32 = (t0 + 1768);
    xsi_vlogvar_assign_value(t32, t31, 0, 0, 25);
    xsi_set_current_line(41, ng0);
    t2 = (t0 + 1608);
    t4 = (t2 + 56U);
    t5 = *((char **)t4);
    memset(t8, 0, 8);
    t6 = (t5 + 4);
    t12 = *((unsigned int *)t6);
    t13 = (~(t12));
    t14 = *((unsigned int *)t5);
    t15 = (t14 & t13);
    t16 = (t15 & 1U);
    if (t16 != 0)
        goto LAB58;

LAB56:    if (*((unsigned int *)t6) == 0)
        goto LAB55;

LAB57:    t7 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t7) = 1;

LAB58:    t9 = (t8 + 4);
    t11 = (t5 + 4);
    t17 = *((unsigned int *)t5);
    t18 = (~(t17));
    *((unsigned int *)t8) = t18;
    *((unsigned int *)t9) = 0;
    if (*((unsigned int *)t11) != 0)
        goto LAB60;

LAB59:    t23 = *((unsigned int *)t8);
    *((unsigned int *)t8) = (t23 & 1U);
    t26 = *((unsigned int *)t9);
    *((unsigned int *)t9) = (t26 & 1U);
    t24 = (t0 + 1608);
    xsi_vlogvar_assign_value(t24, t8, 0, 0, 1);
    goto LAB53;

LAB55:    *((unsigned int *)t8) = 1;
    goto LAB58;

LAB60:    t19 = *((unsigned int *)t8);
    t20 = *((unsigned int *)t11);
    *((unsigned int *)t8) = (t19 | t20);
    t21 = *((unsigned int *)t9);
    t22 = *((unsigned int *)t11);
    *((unsigned int *)t9) = (t21 | t22);
    goto LAB59;

LAB64:    t24 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t24) = 1;
    goto LAB65;

LAB66:    xsi_set_current_line(42, ng0);

LAB69:    xsi_set_current_line(42, ng0);
    t31 = ((char*)((ng1)));
    t32 = (t0 + 1768);
    xsi_vlogvar_assign_value(t32, t31, 0, 0, 25);
    xsi_set_current_line(42, ng0);
    t2 = (t0 + 1608);
    t4 = (t2 + 56U);
    t5 = *((char **)t4);
    memset(t8, 0, 8);
    t6 = (t5 + 4);
    t12 = *((unsigned int *)t6);
    t13 = (~(t12));
    t14 = *((unsigned int *)t5);
    t15 = (t14 & t13);
    t16 = (t15 & 1U);
    if (t16 != 0)
        goto LAB73;

LAB71:    if (*((unsigned int *)t6) == 0)
        goto LAB70;

LAB72:    t7 = (t8 + 4);
    *((unsigned int *)t8) = 1;
    *((unsigned int *)t7) = 1;

LAB73:    t9 = (t8 + 4);
    t11 = (t5 + 4);
    t17 = *((unsigned int *)t5);
    t18 = (~(t17));
    *((unsigned int *)t8) = t18;
    *((unsigned int *)t9) = 0;
    if (*((unsigned int *)t11) != 0)
        goto LAB75;

LAB74:    t23 = *((unsigned int *)t8);
    *((unsigned int *)t8) = (t23 & 1U);
    t26 = *((unsigned int *)t9);
    *((unsigned int *)t9) = (t26 & 1U);
    t24 = (t0 + 1608);
    xsi_vlogvar_assign_value(t24, t8, 0, 0, 1);
    goto LAB68;

LAB70:    *((unsigned int *)t8) = 1;
    goto LAB73;

LAB75:    t19 = *((unsigned int *)t8);
    t20 = *((unsigned int *)t11);
    *((unsigned int *)t8) = (t19 | t20);
    t21 = *((unsigned int *)t9);
    t22 = *((unsigned int *)t11);
    *((unsigned int *)t9) = (t21 | t22);
    goto LAB74;

}


extern void work_m_10673958383904475558_3630433232_init()
{
	static char *pe[] = {(void *)Initial_28_0,(void *)Always_34_1};
	xsi_register_didat("work_m_10673958383904475558_3630433232", "isim/TEST_TOP_isim_beh.exe.sim/work/m_10673958383904475558_3630433232.didat");
	xsi_register_executes(pe);
}
